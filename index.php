...
function bootstrapped_contact_form($atts,$content=null){
  ...
  if(isset($atts['subject'])){
    ...
    if(count($subjects)==1) $subject = $subjects[0];
  }else{
    $subject = __('Information request sent from','bootstrapped-contactform')." ".get_permalink( $post->ID );
  }
  ...
  if( $_POST && is_email( $_POST['email'] ) && $_POST['f_name'] && $_POST['f_msg'] && wp_verify_nonce( $_POST['contactsecurity'], 'contactform' )) {
    $headers = array('From: '.$_POST['f_name'].' <'.$_POST['email'].'>');
...
<form method="post" id="contactform"  class="<?php echo $class;?>"  role="form">
  ...
  <?php if(is_array($subjects) && count($subjects)>1):?>
  ...
  <div class="form-group">
    ...
    <div class="<?php if($class=='form-horizontal') { echo 'col-sm-9';}?>">
      <input type="text" value="<?php echo $current_user->display_name; ?>" name="f_name" class="form-control required " />
    </div>
  </div>
  <div class="form-group">
    ...
    <div class="<?php if($class=='form-horizontal') { echo 'col-sm-9';}?>">
      <input type="text" value="<?php echo $current_user->user_email; ?>" name="email"  class="form-control required email " />
    </div>
  </div>
  <div class="form-group">
    ...
    <div class="<?php if($class=='form-horizontal') { echo 'col-sm-9';}?>">
      <textarea name="f_msg" class="form-control required " style="height:200px"></textarea>
    </div>
  </div>
  <div class="form-group">
